...
class CreateRequest extends FormRequest
{
    ...
    public function rules() : array
    {
        return [
            ...
            'boats.*.boat_id'                            => 'required',
            'boats.*.start_date'                         => 'required|date',
            'boats.*.end_date'                           => 'required|date|after_or_equal:boats.*.start_date',
            'boats.*.schedule_times'                     => 'required|array',
            'boats.*.schedule_times.*.start_time'        => 'required',
            'boats.*.schedule_times.*.end_time'          => 'required',
            'boats.*.closing_type'                       => 'nullable',
            'boats.*.closing_dates'                      => 'nullable|array',
            'boats.*.closing_days'                       => 'nullable|array',
            'boats.*.additional_options'                 => 'nullable|array',
            'boats.*.additional_options.*.option'        => 'required',
            'boats.*.additional_options.*.option.id'     => 'required|integer',
            'boats.*.additional_options.*.selling_price' => 'required|numeric',
            'boats.*.additional_options.*.net_price'     => 'required|numeric',
